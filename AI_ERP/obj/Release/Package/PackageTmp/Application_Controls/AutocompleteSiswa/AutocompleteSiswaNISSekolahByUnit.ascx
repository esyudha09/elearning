<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="AutocompleteSiswaNISSekolahByUnit.ascx.cs" Inherits="AI_ERP.Application_Controls.AutocompleteSiswa.AutocompleteSiswaNISSekolahByUnit" %>
    ...
    function <%=txtNama.ClientID%>_SHOW_AUTOCOMPLETE(){
        $('[id*=<%=txtNama.ClientID%>]').typeahead({
            ...
            source: function (request, response) {
                $.ajax({
                    url: '<%=ResolveUrl("~/APIs/Master/Siswa.svc/ShowAutocompleteNISSekolahByUnitByTahunAjaran?kata_kunci=") %>' + request +
                         (KodeUnitSiswa().value.trim() !== "" ? "&rel_unit=" + KodeUnitSiswa().value.trim() : "") +
                         (TahunAjaran().value.trim() !== "" ? "&tahun_ajaran=" + TahunAjaran().value.trim() : "") +
                         (TahunAjaran().value.trim() !== "" ? "&semester=" + Semester().value.trim() : ""),
                    dataType: "json",
                    type: "GET",
