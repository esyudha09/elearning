...
{
    public partial class wf_NonSekolah : System.Web.UI.Page
    {
        public const string SessionViewDataName = "PAGEDATANONSEKOLAH";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.Master.HeaderTittle = "<img style=\"height: 28px; width: 28px; display: initial;\" src=\"" + ResolveUrl("~/Application_CLibs/images/svg/020-office.svg") + "\">" +
                                       "&nbsp;&nbsp;" +
                                       "Unit Non Sekolah";
        ...
        private void BindListView(bool isbind = true, string keyword = "")
        {
            ...
            if (keyword.Trim() != "")
            {
                ...
                sql_ds.SelectCommand = DAO_Unit.SP_SELECT_ALL_FOR_SEARCH;
            }
            else
            {
                sql_ds.SelectParameters.Clear();
                sql_ds.SelectCommand = DAO_Unit.SP_SELECT_ALL;
            }
        ...
        protected void lnkOKHapus_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Value.Trim() != "")
                {
                    DAO_Unit.Delete(txtID.Value, Libs.LOGGED_USER_M.UserID);
        ...
        protected void lnkOKInput_Click(object sender, EventArgs e)
        {
            try
            {
                AI_ERP.Application_Entities.Unit m = new Application_Entities.Unit();
                ...
                if (txtID.Value.Trim() != "")
                {
                    ...
                    DAO_Unit.Update(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    txtKeyAction.Value = JenisAction.DoUpdate.ToString();
                }
                else
                {
                    DAO_Unit.Insert(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    InitFields();
        ...
        protected void btnShowDetail_Click(object sender, EventArgs e)
        {
            if (txtID.Value.Trim() != "")
            {
                AI_ERP.Application_Entities.Unit m = DAO_Unit.GetByID_Entity(txtID.Value.Trim());
                if (m != null)
                {
                    if (m.Nama != null)
                    {
