...
namespace AI_ERP.Application_Modules.EDUCATION.Penilaian.TK
{
    ...
    {
        public const string SessionViewDataName = "PENGATURANEKSKUL_TK";
        ...
        protected void ListDropdown()
        {
            ...
            Sekolah sekolah = DAO_Sekolah.GetAll_Entity().FindAll(
                m => m.UrutanJenjang == (int)Libs.UnitSekolah.TK).FirstOrDefault();
            if (sekolah != null)
            {
        ...
        {
            Sekolah unit = DAO_Sekolah.GetAll_Entity().FindAll(m => m.UrutanJenjang == (int)Libs.UnitSekolah.TK).FirstOrDefault();
        ...
        protected void ShowListKelas(string tahun_ajaran, string semester, string rel_kelas)
        {
        ...
        public static string GetHTMLListMenuPeserta(string kode, string ctrl_id, string txt_kelasdet_id, string btn_show_id)
        {
            string s_html = "";
            Rapor_DesignEkskul m = DAO_Rapor_DesignEkskul.GetByID_Entity(kode);
            if (m != null)
            {
                if (m.TahunAjaran != null)
                {
                    List<KelasDet> lst_kelas = DAO_KelasDet.GetByKelas_Entity(m.Rel_Kelas.ToString()).OrderBy(m0 => m0.UrutanKelas).ToList();
                    foreach (var item in lst_kelas)
                    {
                        if (item.IsAktif)
                        {
                            s_html += (
                                      ...
                                      "<li style=\"background-color: white; padding: 10px;\">" +
                                        "<label " +
                                            " onclick=\"" + ctrl_id + ".value = '" + kode + "'; " + txt_kelasdet_id + ".value = '" + item.Kode.ToString() + "'; " + btn_show_id + ".click();\"" + "style=\"color: #114D79; cursor: pointer; padding-left: 5px; padding-right: 5px; font-weight: bold;\">" +
                                                "<i class=\"fa fa-folder-o\"></i>&nbsp;&nbsp;" + item.Nama +
                                        "</label>" +
                                      "</li>";
                        }
                    }
        ...
        protected void btnShowPilihPeserta_Click(object sender, EventArgs e)
        {
            ShowListSiswaShowPilihEkskul();
            txtKeyAction.Value = JenisAction.DoShowPilihSiswaEkskul.ToString();
        }
        ...
        protected void ShowListSiswaShowPilihEkskul()
        {
            ltrPilihSiswaPeserta.Text = "";
            if (txtID.Value.Trim() != "")
            {
                var m = DAO_Rapor_DesignEkskul.GetByID_Entity(txtID.Value);
                if (m != null)
                {
                    if (m.TahunAjaran != null)
                    {
                        ...
                        string rel_kelas_det = txtIDKelasDet.Value;
                        ...
                        var lst_rapor_design_ekskul = DAO_Rapor_DesignDetEkskul.GetByTABySMByKelasByMapel_Entity(
                                m.TahunAjaran, m.Semester, m.Rel_Kelas, m.Rel_Mapel
                            );
                        ...
                        if (m_sekolah != null)
                        {
                            if (m_sekolah.Nama != null)
                            {
                                ...
                                foreach (Siswa m_siswa in lst_siswa)
                                {
                                    ...
                                    ltrPilihSiswaPesertaCaption.Text = "Pilih Siswa";
                                    ...
                                    if (m_kelas != null)
                                    {
                                        if (m_kelas.Nama != null)
                                        {
                                            ltrPilihSiswaPesertaCaption.Text = "<div style=\"width: 100%; background-color: #f1f9f7; border-color: #e0f1e9; color: #1d9d74; padding: 15px; border-width: 1px; border-style: solid; border-radius: 5px; font-weight: normal;\">" +
                                    ...
                                    string s_checked = (
                                            lst_rapor_design_ekskul.FindAll(m0 => m0.Rel_Siswa.ToString().ToUpper() == m_siswa.Kode.ToString().ToUpper()).Count > 0
                                            ? " checked=\"checked\" "
                                            : ""
                                        );
                                    ltrPilihSiswaPeserta.Text +=
        ...
        protected void lnkOKPilihSiswaPeserta_Click(object sender, EventArgs e)
        {
            if (txtListIDItemSiswaEkskul.Value.Trim() != "")
            {
                var m = DAO_Rapor_DesignEkskul.GetByID_Entity(txtID.Value);
                if (m != null)
                {
                    if (m.TahunAjaran != null)
                    {
                        var m_design = DAO_Rapor_Design.GetAll_Entity().FindAll(
                                m0 => m0.TahunAjaran == m.TahunAjaran &&
                                      m0.Semester == m.Semester &&
                                      m0.TipeRapor.ToUpper().Trim() == GetTR().Trim() && 
                                      m0.Rel_Kelas == new Guid(m.Rel_Kelas)
                            ).FirstOrDefault();
                        if (m_design != null)
                        {
                            if (m_design.TahunAjaran != null)
                            {
                                DAO_Rapor_DesignDetEkskul.DeletePerKelas(
                                        m_design.Kode.ToString(),
                                        m.Kode.ToString(),
                                        txtIDKelasDet.Value
                                    );
                                string[] arr_id_siswa_ekskul = txtListIDItemSiswaEkskul.Value.Split(new string[] { ";" }, StringSplitOptions.RemoveEmptyEntries);
                                foreach (var item in arr_id_siswa_ekskul)
                                {
                                    DAO_Rapor_DesignDetEkskul.Insert(
                                            new Rapor_DesignDetEkskul {
                                                Kode = Guid.NewGuid(),
                                                Rel_Rapor_Design = m_design.Kode,
                                                Poin = "",
                                                Rel_KomponenRapor = m.Kode,
                                                JenisKomponen = JenisKomponenRapor.MapelEkskul,
                                                Rel_KelasDet = txtIDKelasDet.Value,
                                                Rel_Siswa = item
                                            },
                                            Libs.LOGGED_USER_M.UserID
                                        );
                        ...
                        txtKeyAction.Value = "Desain rapor untuk tahun ajaran " + m.TahunAjaran +
                                             ", semester " + m.Semester + " belum dibuat.";
                    }
                }   
            }
        }
