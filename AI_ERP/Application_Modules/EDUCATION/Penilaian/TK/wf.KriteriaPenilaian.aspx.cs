...
{
    public partial class wf_KriteriaPenilaian : System.Web.UI.Page
    {
        public const string SessionViewDataName = "PAGEDATAKRITERIAPENILAIAN_TK";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.Master.HeaderTittle = "<img style=\"height: 28px; width: 28px; display: initial;\" src=\"" + ResolveUrl("~/Application_CLibs/images/svg/vocabulary.svg") + "\">" +
                                       "&nbsp;&nbsp;" +
                                       "Kriteria Penilaian";
        ...
        private void InitKeyEventClient()
        {
            ...
            txtAlias.Attributes.Add("onkeydown", sKeyEnter + "TinyMCENamaFocus(); return false; }");
            txtNama.Attributes.Add("onkeydown", sKeyEnter + "document.getElementById('" + lnkOKInput.ClientID + "').click(); return false; }");
        }
        ...
        private void BindListView(bool isbind = true, string keyword = "")
        {
            ...
            if (keyword.Trim() != "")
            {
                ...
                sql_ds.SelectCommand = DAO_Rapor_Kriteria.SP_SELECT_ALL_FOR_SEARCH;
            }
            else
            {
                sql_ds.SelectParameters.Clear();
                sql_ds.SelectCommand = DAO_Rapor_Kriteria.SP_SELECT_ALL;
            }
        ...
        protected void lnkOKHapus_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Value.Trim() != "")
                {
                    DAO_Rapor_Kriteria.Delete(txtID.Value, Libs.LOGGED_USER_M.UserID);
        ...
        protected void lnkOKInput_Click(object sender, EventArgs e)
        {
            try
            {
                Rapor_Kriteria m = new Rapor_Kriteria();
                ...
                if (txtID.Value.Trim() != "")
                {
                    ...
                    DAO_Rapor_Kriteria.Update(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    txtKeyAction.Value = JenisAction.DoUpdate.ToString();
                }
                else
                {
                    DAO_Rapor_Kriteria.Insert(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    InitFields();
        ...
        protected void btnShowDetail_Click(object sender, EventArgs e)
        {
            if (txtID.Value.Trim() != "")
            {
                Rapor_Kriteria m = DAO_Rapor_Kriteria.GetByID_Entity(txtID.Value.Trim());
                if (m != null)
                {
                    if (m.Nama != null)
                    {
