...
{
    public partial class wf_KategoriPencapaian : System.Web.UI.Page
    {
        public const string SessionViewDataName = "PAGEDATAKATEGORIPENCAPAIAN_KB";
        ...
        private void BindListView(bool isbind = true, string keyword = "")
        {
            ...
            if (keyword.Trim() != "")
            {
                ...
                sql_ds.SelectCommand = DAO_Rapor_KategoriPencapaian.SP_SELECT_ALL_FOR_SEARCH;
            }
            else
            {
                sql_ds.SelectParameters.Clear();
                sql_ds.SelectCommand = DAO_Rapor_KategoriPencapaian.SP_SELECT_ALL;
            }
        ...
        protected void lnkOKHapus_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Value.Trim() != "")
                {
                    DAO_Rapor_KategoriPencapaian.Delete(txtID.Value, Libs.LOGGED_USER_M.UserID);
        ...
        protected void lnkOKInput_Click(object sender, EventArgs e)
        {
            try
            {
                Rapor_KategoriPencapaian m = new Rapor_KategoriPencapaian();
                m.Nama = txtNamaVal.Value;
                m.Keterangan = txtKeterangan.Text;
                if (txtID.Value.Trim() != "")
                {
                    m.Kode = new Guid(txtID.Value);
                    DAO_Rapor_KategoriPencapaian.Update(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    txtKeyAction.Value = JenisAction.DoUpdate.ToString();
                }
                else
                {
                    DAO_Rapor_KategoriPencapaian.Insert(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    InitFields();
        ...
        protected void btnShowDetail_Click(object sender, EventArgs e)
        {
            if (txtID.Value.Trim() != "")
            {
                Rapor_KategoriPencapaian m = DAO_Rapor_KategoriPencapaian.GetByID_Entity(txtID.Value.Trim());
                if (m != null)
                {
                    if (m.Nama != null)
                    {
