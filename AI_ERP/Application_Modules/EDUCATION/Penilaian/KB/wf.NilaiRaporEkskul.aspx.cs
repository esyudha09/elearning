...
namespace AI_ERP.Application_Modules.EDUCATION.Penilaian.KB
{
    public partial class wf_NilaiRaporEkskul : System.Web.UI.Page
    {
        ...
        {
            if (txtID.Value.Trim() == "") return DAO_Siswa.GetByRombel_Entity(
                         QS.GetUnit(),
                         txtKelasDet.Value,
                         tahun_ajaran,
                         semester
        ...
        public static string GetHTMLRdoKriteriaPenilaian(string tahun_ajaran, string semester, string kelas_det, string rel_siswa, string rel_rapor_desain, string kode_item_desain, string rel_rapor_kriteria, string rel_mapel, AI_ERP.Application_Entities.Elearning.KB.JenisKomponenRapor jenis, bool align_right = true)
        {
            string hasil = "";
                ...
                if (kode_nilai_siswa.Trim() != "" && rel_siswa.Trim() != "")
                {
                    ...
                    if (lst_nilai_siswa_det.Count > 0)
                    {
                        if (lst_nilai_siswa_det.FirstOrDefault().Deskripsi != null)
                        {
                            ...
                            return Libs.GetHTMLNoParagraphDiAwal(
                                        hasil,
                                        false
                                    );
                        }
