...
{
    public partial class wf_PoinKategoriPencapaian : System.Web.UI.Page
    {
        public const string SessionViewDataName = "PAGEDATAPOINKATEGORIPENCAPAIAN_KB";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.Master.HeaderTittle = "<img style=\"height: 28px; width: 28px; display: initial;\" src=\"" + ResolveUrl("~/Application_CLibs/images/svg/test.svg") + "\">" +
                                       "&nbsp;&nbsp;" +
        ...
        private void BindListView(bool isbind = true, string keyword = "")
        {
            ...
            if (keyword.Trim() != "")
            {
                ...
                sql_ds.SelectCommand = DAO_Rapor_PoinKategoriPencapaian.SP_SELECT_ALL_FOR_SEARCH;
            }
            else
            {
                sql_ds.SelectParameters.Clear();
                sql_ds.SelectCommand = DAO_Rapor_PoinKategoriPencapaian.SP_SELECT_ALL;
            }
        ...
        protected void lnkOKHapus_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Value.Trim() != "")
                {
                    DAO_Rapor_PoinKategoriPencapaian.Delete(txtID.Value, Libs.LOGGED_USER_M.UserID);
        ...
        protected void lnkOKInput_Click(object sender, EventArgs e)
        {
            try
            {
                Rapor_PoinKategoriPencapaian m = new Rapor_PoinKategoriPencapaian();
                m.Nama = txtNamaVal.Value;
                m.Keterangan = txtKeterangan.Text;
                if (txtID.Value.Trim() != "")
                {
                    m.Kode = new Guid(txtID.Value);
                    DAO_Rapor_PoinKategoriPencapaian.Update(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    txtKeyAction.Value = JenisAction.DoUpdate.ToString();
                }
                else
                {
                    DAO_Rapor_PoinKategoriPencapaian.Insert(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    InitFields();
        ...
        protected void btnShowDetail_Click(object sender, EventArgs e)
        {
            if (txtID.Value.Trim() != "")
            {
                Rapor_PoinKategoriPencapaian m = DAO_Rapor_PoinKategoriPencapaian.GetByID_Entity(txtID.Value.Trim());
                if (m != null)
                {
                    if (m.Nama != null)
                    {
