...
namespace AI_ERP.Application_Modules.EDUCATION.Penilaian.KB
{
    ...
    {
        public const string SessionViewDataName = "PAGEDATANILAIRAPOR_KB";
        ...
        public static bool IsReadonly(string tahun_ajaran, string semester, string rel_kelasdet, string rel_siswa, string jenis_rapor)
        {
            var nilai_log = DAO_Rapor_UploadHist.Siswa.GetStatusUpload(
                    tahun_ajaran, semester, rel_kelasdet, rel_siswa, jenis_rapor
                );
        ...
        protected void InitURLOnMenu()
        {
            ...
            else
            {
                ...
                if (mvMain.ActiveViewIndex == 1)
                {
                    if (IsReadonly(txtTahunAjaran.Value, txtSemester.Value, txtKelasDet.Value, txtIDSiswa.Value, txtJenisRapor.Value)) s_background = "#992f52";
                }
        ...
        protected void ShowListSiswa()
        {
                                            ...
                                            "<table style=\"margin: 0px; width: 100%;\">" +
                                                "<tr>" +
                                                    ...
                                                    "<td style=\"background-color: white; padding: 0px; font-size: small; padding-top: 7px;\">" +
                                                        "<span style=\"color: " +
                                                            (
                                                                IsReadonly(tahun_ajaran, semester, txtKelasDet.Value, m_siswa.Kode.ToString(), "")
                                                                ? "#b0315c; "
                                                                : "#295BC8; "
                                                            )
                                                            + "font-weight: bold;\">" +
            ...
            string s_url = (
                            "'" +
                                ResolveUrl(
                                    Routing.URL.APPLIACTION_MODULES.EDUCATION.Penilaian.KB.NILAI_SISWA_PRINT.ROUTE +
        ...
        protected void ShowDataSiswa()
        {
                ...
                if (m_siswa != null)
                {
                    if (m_siswa.Nama != null)
                    {
                        ...
                        if (m_nilai_siswa != null)
                        {
                            ...
                            {
                                txtIDNilaiSiswa.Value = m_nilai_siswa.Kode.ToString();
                                ...
                                txtUsia.Text = m_nilai_siswa.Usia;
                            }
                        }
                    }
                }
            ...
            if (Libs.URL_IDENTIFIER.IsAdaIDUrlIdAdminUnit())
            {
                if (AI_ERP.Application_Modules.EDUCATION.Penilaian.KB.wf_NilaiRapor.IsReadonly(
                    txtTahunAjaran.Value,
                    txtSemester.Value,
                    txtKelasDet.Value,
                    txtIDSiswa.Value,
                ...
                ))
                {
                    ...
                    txtLingkarKepala.Enabled = false;
                    txtUsia.Enabled = false;
                }
                else
                {
                    txtBeratBadan.Enabled = true;
                    txtTinggiBadan.Enabled = true;
                    txtLingkarKepala.Enabled = true;
                    txtUsia.Enabled = true;
                }
            }
            else
            {
                bool is_posted = IsReadonly(tahun_ajaran, semester, txtKelasDet.Value, txtIDSiswa.Value, txtJenisRapor.Value);
                txtBeratBadan.Enabled = !is_posted;
                txtTinggiBadan.Enabled = !is_posted;
                txtLingkarKepala.Enabled = !is_posted;
                txtUsia.Enabled = !is_posted;
            }
        }
        ...
        public static string GetHTMLRdoKriteriaPenilaian(string tahun_ajaran, string semester, string kelas_det, string rel_siswa, string rel_rapor_desain, string kode_item_desain, string rel_rapor_kriteria, AI_ERP.Application_Entities.Elearning.KB.JenisKomponenRapor jenis, bool align_right = true, string as_readonly = "")
        {
            string hasil = "";
            Rapor_Design m_design = DAO_Rapor_Design.GetByID_Entity(rel_rapor_desain);
            List<Rapor_DesignKriteria> lst_desain_kriteria = DAO_Rapor_DesignKriteria.GetByHeader_Entity(rel_rapor_desain).OrderBy(
                    m => m.Urut
                ).ToList();
            bool is_posted = IsReadonly(tahun_ajaran, semester, kelas_det, rel_siswa, m_design.JenisRapor);
            ...
            foreach (Rapor_DesignKriteria kriteria in lst_desain_kriteria)
            {
                ...
                //if (jenis == AI_ERP.Application_Entities.Elearning.KB.JenisKomponenRapor.PoinKategoriPencapaian)
                if (jenis != AI_ERP.Application_Entities.Elearning.KB.JenisKomponenRapor.Rekomendasi)
                {
                    //get data
                    string s_checked = "";
                    s_checked = (
                                    ...
                                    "<div class=\"checkbox checkbox-adv\" style=\"" + (align_right ? " float: right; " : "") + "\">" +
                                        "<label style=\"color: grey;\" for=\"chk_desain_" + kode_item_desain.Replace("-", "_") + "_" + id.ToString() + "\">" +
                                            "<input value=\"" + kriteria.Kode.ToString() + "\" " +
                                                   ...
                                                   //"name=\"chk_desain_" + id.ToString() + "[]\" " +
                                                   "lang=\"" + kode_item_desain + "\" " +
                                                   "type=\"checkbox\" />" +
                ...
                else if (jenis == AI_ERP.Application_Entities.Elearning.KB.JenisKomponenRapor.Rekomendasi)
                {
                    hasil += "";
                }
                else
        ...
        public static string GetHTMLRdoKriteriaPenilaianEkskul(string tahun_ajaran, string semester, string kelas_det, string rel_siswa, string rel_rapor_desain, string kode_item_desain, string rel_rapor_kriteria, AI_ERP.Application_Entities.Elearning.KB.JenisKomponenRapor jenis, bool align_right = true)
        {
            string hasil = "";
            Rapor_Design m_design = DAO_Rapor_Design.GetByID_Entity(rel_rapor_desain);
            List<Rapor_DesignKriteria> lst_desain_kriteria = DAO_Rapor_DesignKriteria.GetByHeader_Entity(rel_rapor_desain).OrderBy(
        ...
        public static string GetHTMLDeskripsiPenilaian(string tahun_ajaran, string semester, string kelas_det, string rel_siswa, string rel_rapor_desain, string kode_item_desain, AI_ERP.Application_Entities.Elearning.KB.JenisKomponenRapor jenis, bool align_right = true)
        {
            string hasil = "";
        ...
        private void BindListView(bool isbind = true, string keyword = "")
        {
            ...
            if (Libs.URL_IDENTIFIER.IsAdaIDUrlIdAdminUnit())
            {
                ...
                if (keyword.Trim() != "")
                {
                    ...
                    sql_ds.SelectParameters.Add("TipeRapor", QS.GetTR());
            ...
            if (QS.GetTR() == TipeRapor.LTS)
            {
                ltrTipeRapor.Text = "<span style=\"font-weight: bold;\">LTS</span>";
            }
            else if (QS.GetTR() == TipeRapor.SEMESTER)
            {
        ...
        private void BindListViewDesain(string rel_desain_rapor, string rel_siswa, bool isbind = true)
        {
            ...
            if (m != null)
            {
                if (m.TahunAjaran != null)
                {
                    ...
                    if (IsReadonly(m.TahunAjaran, m.Semester, s_kelas, txtIDSiswa.Value, txtJenisRapor.Value))
                    {
            ...
            if (txtID.Value.Trim() != "")
            {
                ...
                if (m != null)
                {
                    if (m.TahunAjaran != null)
                    {
                        ...
                        txtJenisRapor.Value = m.JenisRapor;
        ...
        public static int GetJumlahNoReadOnly(string rel_kelasdet, string rel_designrapor)
        {
            ...
            if (m_rapor_design != null)
            {
                if (m_rapor_design.TahunAjaran != null)
                {
                    ...
                    {
                        if (!IsReadonly(m_rapor_design.TahunAjaran, m_rapor_design.Semester, rel_kelasdet, siswa.Kode.ToString(), m_rapor_design.JenisRapor)) jml_not_readonly++;
                    }
        ...
        public static string GetHTMLNotifJumlahSiswa(string rel_kelasdet, string rel_designrapor)
        {
            int jml_not_readonly = GetJumlahNoReadOnly(rel_kelasdet, rel_designrapor);
