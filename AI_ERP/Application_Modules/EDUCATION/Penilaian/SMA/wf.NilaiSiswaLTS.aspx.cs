...
namespace AI_ERP.Application_Modules.EDUCATION.Penilaian.SMA
{
    public partial class wf_NilaiSiswaLTS : System.Web.UI.Page
    {
        ...
        public string s_url_3 = "";
        public string s_url_ok = AI_ERP.Application_Libs.Routing.URL.APPLIACTION_MODULES.EDUCATION.Penilaian.SMP.CETAK_LTS.ROUTE;
                ...
                if (m_siswa != null)
                {
                    if (m_siswa.Nama != null)
                    {
                        ...
                        lnkOKCetakAktif.Attributes["onclick"] = "window.open('" + s_url_2 + "', '_blank', 'left=0,top=0,width=800,height=900,toolbar=0,scrollbars=0,status=0;'); " +
                                                                "return false; ";
                        lnkOKDeskripsiLTS.Attributes["onclick"] = "window.open('" + s_url_3 + "', '_blank', 'left=0,top=0,width=800,height=900,toolbar=0,scrollbars=0,status=0;'); " +
                                                                  "return false; ";

                        Kelas m_kelas = DAO_Kelas.GetByID_Entity(DAO_Kelas.GetByID_Entity(DAO_KelasDet.GetByID_Entity(m_siswa.Rel_KelasDet).Rel_Kelas.ToString()).Kode.ToString());
                        Reports_SMA.LTS lts = new Reports_SMA.LTS(QS.GetTahunAjaran(), QS.GetSemester(), QS.GetKelas(), m_siswa.Kode.ToString(), "");
                        ltrLTS.Text = lts.GetHTML;
