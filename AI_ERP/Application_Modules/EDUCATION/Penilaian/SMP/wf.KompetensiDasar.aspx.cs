...
{
    public partial class wf_KompetensiDasar : System.Web.UI.Page
    {
        public const string SessionViewDataName = "PAGEDATAKOMPETENSIDASAR_SMP";
        ...
        private void BindListView(bool isbind = true, string keyword = "")
        {
            ...
            if (keyword.Trim() != "")
            {
                ...
                sql_ds.SelectCommand = DAO_Rapor_KompetensiDasar.SP_SELECT_ALL_FOR_SEARCH;
            }
            else
            {
                sql_ds.SelectParameters.Clear();
                sql_ds.SelectCommand = DAO_Rapor_KompetensiDasar.SP_SELECT_ALL;
            }
        ...
        protected void lnkOKHapus_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Value.Trim() != "")
                {
                    DAO_Rapor_KompetensiDasar.Delete(txtID.Value, Libs.LOGGED_USER_M.UserID);
        ...
        protected void lnkOKInput_Click(object sender, EventArgs e)
        {
            try
            {
                Rapor_KompetensiDasar m = new Rapor_KompetensiDasar();
                ...
                if (txtID.Value.Trim() != "")
                {
                    ...
                    DAO_Rapor_KompetensiDasar.Update(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    txtKeyAction.Value = JenisAction.DoUpdate.ToString();
                }
                else
                {
                    DAO_Rapor_KompetensiDasar.Insert(m, Libs.LOGGED_USER_M.UserID);
                    BindListView(!IsPostBack, Libs.GetQ().Trim());
                    InitFields();
        ...
        protected void btnShowDetail_Click(object sender, EventArgs e)
        {
            if (txtID.Value.Trim() != "")
            {
                Rapor_KompetensiDasar m = DAO_Rapor_KompetensiDasar.GetByID_Entity(txtID.Value.Trim());
                if (m != null)
                {
                    if (m.Nama != null)
                    {
