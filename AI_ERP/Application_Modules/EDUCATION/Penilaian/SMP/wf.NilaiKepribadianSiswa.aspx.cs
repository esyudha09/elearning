...
{
    public partial class wf_NilaiKepribadianSiswa : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.Master.HeaderTittle = "<i class=\"fa fa-users\" style=\"font-size: 16pt\"></i>" +
                                       "&nbsp;&nbsp;" +
                                       "Kepribadian Siswa";
                            ...
                            if (m_sekolah != null)
                            {
                                if (m_sekolah.Nama != null)
                                {
                                    ...
                                    foreach (Siswa m_siswa in lst_siswa)
                                    {
                                        ...
                                        Rapor_Kepribadian m_catatan_siswa = DAO_Rapor_Kepribadian.GetAllByTABySMByKelasDetSiswa_Entity(
                                        ...
                                        if (m_catatan_siswa != null)
                                        {
                                            ...
                                            {
                                                catatan = "Kelakuan : " + "<label style=\"font-weight: bold;\">" + m_catatan_siswa.Kelakuan + "</label>" + "<br />" +
                                                          "Kerajinan : " + "<label style=\"font-weight: bold;\">" + m_catatan_siswa.Kerajinan + "</label>" + "<br />" +
                                                          "Kerapihan : " + "<label style=\"font-weight: bold;\">" + m_catatan_siswa.Kerapihan + "</label>" + "<br />";
                                            }
                                        }
                                                                        ...
                                                                        "<td colspan=\"3\" style=\"background-color: white; padding: 0px; font-size: small; padding-top: 7px; padding-right: 10px; padding-left: 10px;\">" +
                                                                            "<div class=\"row\" style=\"margin-left: 0px; margin-right: 1px;\">" +
                                                                                "<div onclick=\"" + txtRelSiswa.ClientID + ".value = '" + m_siswa.Kode.ToString() + "'; " + btnShowIsiCatatan.ClientID + ".click();\" class=\"col-xs-12\" " +
                                                                                    ...
                                                                                    "<div style=\"cursor: pointer; color: black; min-height: 50px; padding: 3px;\">" +
                                                                                        (
                                                                                            catatan.Trim() == ""
                                                                                            ? "<span style=\"color: #bfbfbf;\">Kepribadian...</span>"
                                                                                            : catatan.Trim().
                                                                                              Replace("<p>", "").Replace("</p>", "").
                                                                                              Replace("<P>", "").Replace("</P>", "")
                                                                                        ) +
            ...
            if (txtRelSiswa.Value.Trim() != "")
            {
                ...
                if (m_siswa != null)
                {
                    if (m_siswa.Nama != null)
                    {
                        ...
                        List<Rapor_Kepribadian> lst_catatan = DAO_Rapor_Kepribadian.GetAllByTABySMByKelasDetSiswa_Entity(
                                QS.GetTahunAjaran(), QS.GetSemester(), QS.GetKelas(), txtRelSiswa.Value
                            );
                        if (lst_catatan.Count > 0)
                        {
                            Rapor_Kepribadian m_KepribadianSiswa = lst_catatan.FirstOrDefault();
                            if (m_KepribadianSiswa != null)
                            {
                                if (m_KepribadianSiswa.TahunAjaran != null)
                                {
                                    txtKelakuan.Text = m_KepribadianSiswa.Kelakuan;
                                    txtKerajinan.Text = m_KepribadianSiswa.Kerajinan;
                                    txtKerapian.Text = m_KepribadianSiswa.Kerapihan;
                                }
                            }
                        }
        ...
        protected void lnkOKSaveCatatan_Click(object sender, EventArgs e)
        {
            ...
            Rapor_Kepribadian m_KepribadianSiswa = new Rapor_Kepribadian();
            if (lst_catatan.Count > 0)
            {
                m_KepribadianSiswa = lst_catatan.FirstOrDefault();
                m_KepribadianSiswa.Kelakuan = txtKelakuan.Text;
                m_KepribadianSiswa.Kerajinan = txtKerajinan.Text;
                m_KepribadianSiswa.Kerapihan = txtKerapian.Text;
                DAO_Rapor_Kepribadian.Update(m_KepribadianSiswa, Libs.LOGGED_USER_M.UserID);
            }
            else
            {
                m_KepribadianSiswa.Kode = Guid.NewGuid();
                m_KepribadianSiswa.TahunAjaran = QS.GetTahunAjaran();
                m_KepribadianSiswa.Semester = QS.GetSemester();
                m_KepribadianSiswa.Rel_KelasDet = QS.GetKelas();
                m_KepribadianSiswa.Rel_Siswa = txtRelSiswa.Value;
                m_KepribadianSiswa.Kelakuan = txtKelakuan.Text;
                m_KepribadianSiswa.Kerajinan = txtKerajinan.Text;
                m_KepribadianSiswa.Kerapihan = txtKerapian.Text;
                DAO_Rapor_Kepribadian.Insert(m_KepribadianSiswa, Libs.LOGGED_USER_M.UserID);
            }
            ShowListSiswa();
