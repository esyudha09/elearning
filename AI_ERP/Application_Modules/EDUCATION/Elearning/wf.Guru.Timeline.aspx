    ...
    <script type="text/javascript">
        ...
        function InitModalOpen(){
            $('#ui_modal_absensi').on('shown.bs.modal', function () {
                var txt = document.getElementById("<%= txtKeteranganAbsen.ClientID %>");
                if(txt !== null && txt !== undefined){
                    var l = txt.value.trim().length;
                    if(l > 0){
                        document.getElementById("div_keterangan").style.display = "inherit";
                    } else {
        ...
        function ProsesAbsen_V1() {
            ShowprosesAbsen(true);
            
            var div_keterangan = document.getElementById("div_keterangan");
            if(div_keterangan != null && div_keterangan != undefined){
                div_keterangan.style.display = "none";
            }
        ...
        function ProsesAbsen() {
            ...
            var arr_absen = document.getElementsByName("chk_pilih_kedisplinan[]");
            var arr_ket_kedisiplinan = document.getElementsByName("txt_keterangan_kedisplinan[]");
            var arr_ket_absen = document.getElementsByName("txt_keterangan_absen[]");
            var arr_siswa_absen = document.getElementsByName("txt_siswa_absen[]");
            var arr_kejadian = document.getElementsByName("txt_kejadian[]");
            var arr_butir_sikap = document.getElementsByName("cbo_butir_sikap[]");
            ...
            if (txt_absen != null && txt_absen != undefined && txt_id != null && txt_id != undefined) {
                txt_absen.value = "";
                if (
                    arr_ket_absen.length > 0 && 
                    arr_ket_absen.length === arr_siswa_absen.length
                ) {
                    if (txt_jumlah_proses_absen != null && txt_jumlah_proses_absen != undefined)
                    {
                        txt_jumlah_proses_absen.value = arr_ket_absen.length.toString();
                    }
                    ...
                    for (var i = 0; i < arr_ket_absen.length; i++) {
                        var s_absen = "";
                        if(arr_absen.length > 0){
                            for (var j = 0; j < arr_absen.length; j++) {
                                if(arr_absen[j].value === arr_siswa_absen[i].value){
                                    s_absen += arr_absen[j].lang + 
                                               (
                                                    arr_ket_kedisiplinan.length == arr_absen.length
                                                    ? "~" + arr_ket_kedisiplinan[j].value.replaceAll("~", "")
                                                    : "~"
                                               ) + 
                                               ";";
                                }
        ...
        var txt_tanggal = function () { return document.getElementById('<%= txtTanggalAbsenBuka.ClientID %>'); }
        ...
        var txt_nik = function () { return document.getElementById('<%= txtNIK.ClientID %>'); }
        ...
        function ValidateAbsen(){
            ...
            var cbo_jam = document.getElementById("<%= cboJamKe.ClientID %>");
            if(cbo_jam !== null && cbo_jam !== undefined){
                if(txt_mapel().value.trim() !== "" && cbo_jam.value.trim() === ""){
                    alert("Isian \"Jam ke\" pada presensi harus diisi.");
                    cbo_jam.focus();
                    return false;
                }
            }
            ...
            var is_ok = false;
            var s_url = '<%= ResolveUrl(
                            AI_ERP.Application_Libs.Routing.URL.APIS._GENERAL.ABSENSI_SISWA.DO_VALIDATE.FILE
                        ) %>/GetValidateAbsen' + "?";
            s_url += "&tgl=" + txt_tanggal().value;
            s_url += "&m=" + txt_mapel().value;
            s_url += "&kd=" + txt_kelas().value;
            s_url += "&g=" + txt_nik().value;
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                success: function (data) {
                    if(data.d === "<%= AI_ERP.Application_DAOs.Elearning.DAO_SiswaAbsenMapel.JADWAL_ABSEN.DILUAR_JADWAL %>"){
                        div_keterangan.style.display = "inherit";
                        var txt = document.getElementById("<%= txtKeteranganAbsen.ClientID %>");
                        if(txt !== null && txt !== undefined){
                            if(txt.value.trim() === ""){
                                txt.focus();
        ...
        function SetCheckKedisiplinan(chk_id){
        ...
        function SetDisabledCheckKetidakhadiran(value, chk_id_sakit, chk_id_izin, chk_id_alpa){
            var id_sakit = document.getElementById(chk_id_sakit);
            var id_izin = document.getElementById(chk_id_izin);
            var id_alpa = document.getElementById(chk_id_alpa);

            if(
                id_sakit !== undefined && id_sakit !== null &&
                id_izin !== undefined && id_izin !== null &&
                id_alpa !== undefined && id_alpa !== null
              ){
                if(value == true){
                    id_sakit.checked = false; id_sakit.disabled = true;
                    id_izin.checked = false; id_izin.disabled = true;
                    id_alpa.checked = false; id_alpa.disabled = true;
                } else {
                    id_sakit.disabled = false;
                    id_izin.disabled = false;
                    id_alpa.disabled = false;
                }
            }
        }

        function ShowTextboxKeterangan(chk_id, is_show){
            var div = document.getElementById(chk_id.replaceAll("chk", "div"));
            if(div !== null && div !== undefined){
                div.style.display = (is_show === true ? "" : "none");
            }
        }

        function ShowTextboxKeteranganKedisiplinan(div_id, is_show){
            var div_1 = document.getElementById(div_id + "_1");
            var div_2 = document.getElementById(div_id + "_2");

            if(div_1 !== null && div_1 !== undefined){
                div_1.style.display = (is_show === true ? "" : "none");
            }

            if(div_2 !== null && div_2 !== undefined){
                div_2.style.display = (is_show === true ? "" : "none");
            }
        }
    </script>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:UpdatePanel ID="upMain" runat="server">
        <ContentTemplate>
            ...
            <asp:HiddenField runat="server" ID="txtNIK" />
            ...
            <div class="fbtn-container" id="div_button_settings" runat="server" style="z-index: 91;">
		        <div class="fbtn-inner">
                    ...
                    <div class="fbtn-dropup" style=" z-index: 999999;">
                        ...
                        <asp:LinkButton Visible="true" OnClientClick="ShowProgress(true);" CssClass="fbtn fbtn-green waves-attach waves-circle waves-effect" runat="server" ID="lnkOKShowDownloadAbsen" OnClick="lnkOKShowDownloadAbsen_Click" style="background-color: #424242;">
                            <span class="fbtn-text fbtn-text-left">Download Laporan Absen</span>
                            <i class="fa fa-file-excel-o" style="color: white;"></i>
                        </asp:LinkButton>
                        <asp:LinkButton Visible="false" CssClass="fbtn fbtn-green waves-attach waves-circle waves-effect" runat="server" ID="lnkOKDownloadLaporanPresensi" OnClick="lnkOKDownloadLaporanPresensi_Click" style="background-color: green;">
                            <span class="fbtn-text fbtn-text-left">Download Laporan Presensi</span>
                            <i class="fa fa-file-excel-o" style="color: white;"></i>
                        </asp:LinkButton>
                        <asp:LinkButton Visible="false" OnClientClick="ShowProgress(true);" CssClass="fbtn fbtn-green waves-attach waves-circle waves-effect" runat="server" ID="lnkAbsenSiswaLTS" OnClick="lnkAbsenSiswaLTS_Click" style="background-color: #424242;">
                            <span class="fbtn-text fbtn-text-left">Presensi Siswa (LTS)</span>
                            <i class="fa fa-calendar-check-o" style="color: white;"></i>
                        </asp:LinkButton>
                        <asp:LinkButton Visible="false" OnClientClick="ShowProgress(true);" CssClass="fbtn fbtn-green waves-attach waves-circle waves-effect" runat="server" ID="lnkAbsenSiswaRapor" OnClick="lnkAbsenSiswaRapor_Click" style="background-color: #424242;">
                            <span class="fbtn-text fbtn-text-left">Presensi Siswa (Rapor)</span>
                            <i class="fa fa-calendar-check-o" style="color: white;"></i>
                        </asp:LinkButton>
                        <asp:LinkButton OnClientClick="ShowProgress(true);" CssClass="fbtn fbtn-green waves-attach waves-circle waves-effect" runat="server" ID="lnkAbsenSiswa" OnClick="lnkAbsenSiswa_Click" style="background-color: #424242;">
                            <span class="fbtn-text fbtn-text-left">Presensi Siswa</span>
                ...
                <table style="margin: 0px; width: 100%;">
                    <tr>
                        ...
                        <td style="background-color: white; padding: 5px; padding-left: 10px; padding-right: 0px;">
                            <asp:TextBox placeholder="Cari data..." runat="server" ID="txtCari" style="padding: 0px; border-style: none; color: black; outline: none; width: 100%;"></asp:TextBox>
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <div aria-hidden="true" class="modal fade" id="ui_modal_absensi" role="dialog" tabindex="-1" style="display: none; padding-right: 9px; z-index: 2000;">
                                            ...
                                            <div class="row" style="margin-left: 0px; margin-right: 0px;">
                                                <div class="col-xs-12">
                                                    ...
                                                    <div id="div_proses_absen" style="display: none; background-color: white; height: 150px; width: 100%; z-index: 9000; padding-top: 20px;">
                                                        ...
                                                        <label style="margin: 0 auto; display: table; color: black; font-size: medium; font-weight: bold; margin-top: 20px;">Sedang Proses...</label>
                                                    </div>
                                                    ...
                                                    <div id="div_tanggal_buka_absen" class="row" style="margin-bottom: 5px; padding-bottom: 5px;">
                                                        <div class="col-xs-12" style="color: black;">
                                                            ...
                                                            &nbsp;
                                                            <label for="<%= txtTanggalAbsenBuka.ClientID %>" style="font-size: small; color: black; margin-bottom: 10px;">Tanggal Presensi</label>
                                                            &nbsp;
                                                    ...
                                                    <div id="div_tanggal_buka_absen_jamke" runat="server" class="row" style="margin-bottom: 5px; padding-bottom: 5px;">
                                                        <div class="col-xs-12" style="color: black;">
                                                            ...
                                                            &nbsp;
                                                            <label for="<%= cboJamKe.ClientID %>" style="font-size: small; color: black; margin-bottom: 10px;">Jam Ke</label>
                                                    ...
                                                    <div id="div_keterangan" class="row" style="display: none; margin-bottom: 5px; padding-bottom: 5px;">
                                                        <div class="col-xs-12" style="color: black; padding-top: 15px;">
                                                            
                                                            <div style="padding: 10px; background-color: #fc7e00; color: white; font-weight: bold;">
                                                                <i class="fa fa-exclamation-triangle"></i>&nbsp;&nbsp;
                                                                Pengisian absensi diluar jadwal yang ditentukan, harap isi keterangan sebelum menyimpan data
                                                            </div>
                                                            <asp:TextBox runat="server" ID="txtKeteranganAbsen" Rows="1" TextMode="MultiLine" style="border-style: solid; border-color: #bfbfbf; background-color: #F5F8FA; border-width: 1px; width: 100%; resize: none; border: 1px solid #E6ECF0; outline: none; padding: 10px; font-weight: bold; color: black;"></asp:TextBox>
                ...
                <div class="fbtn-container" id="div_footer_dimpan_absen" runat="server" style="position: fixed; right: 25px; bottom: 5px;">
		            <div class="fbtn-inner">
			            <a onclick="ValidateAbsen(); return false;" class="fbtn fbtn-lg fbtn-brand-accent waves-attach waves-circle waves-light" data-toggle="dropdown" style="background-color: #329CC3;" title=" Simpan ">
                ...
                <div class="content-header ui-content-header" 
					...
					<div style="padding-left: 15px;">
						<a onclick="setTimeout(function() { ShowPengaturan(true); }, 500);" class="btn btn-flat btn-brand-accent waves-attach waves-effect" data-dismiss="modal" style="margin-left: 0px; margin-right: 0px; color: black;">
                                                ...
                                                <div class="col-xs-12">
                                                    <div class="row" style="margin-bottom: 5px; padding-bottom: 5px;">
                                                        <div class="col-xs-12" style="color: black;">
                                                            ...
                                                            <i class="fa fa-calendar"></i>
                                                            &nbsp;
                                                            <label for="<%= txtTanggalAbsen.ClientID %>" style="font-size: small; color: black; margin-bottom: 10px;">Tanggal Presensi</label>
                                                            &nbsp;
        ...
        <Triggers>
            <asp:PostBackTrigger ControlID="lnkOKDownloadLaporanPresensi" />
        </Triggers>
    </asp:UpdatePanel>
